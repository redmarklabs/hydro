...
1. Component's view model is set to be the Hydro component. This is because all the state lays there. If you want to extract the model to a separate file, you can reference it as one property on the component.
...
4. Use the `on` attribute to attach browser events to the component's methods. [Read more here](actions).

## Usage

To use your new component, you can render it in your Razor Page (e.g. `Index.cshtml`) or in another Hydro component. There are several ways to do this:
...
`Mount` is called only once when component is instantiated on the page.

### Render or RenderAsync

`Render` is called on each request to render this component. You can use it to pass additional data calculated on the fly, or to get temporary data from the database that you don't want to store in the component's persistent state.
...
`Handle` method is used here to catch the custom event `SystemMessageEvent` that we subscribed to in the constructor. It's called whenever this type of event is triggered in the application.
